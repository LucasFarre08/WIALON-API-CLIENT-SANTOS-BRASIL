...
REM ==== Define data fixa para teste ====
set "DATA=2026-01-01"

REM ==== Define intervalo com base na data informada ====
for /f "delims=" %%I in ('
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "[datetime]::Parse('%DATA%').ToString('yyyy-MM-dd 00:00:00')"
') do set "FROM=%%~I"

for /f "delims=" %%I in ('
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "[datetime]::Parse('%DATA%').AddHours(23).AddMinutes(59).AddSeconds(59).ToString('yyyy-MM-dd HH:mm:ss')"
') do set "TO=%%~I"
